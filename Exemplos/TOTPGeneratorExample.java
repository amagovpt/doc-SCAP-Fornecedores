package pt.ama.scap.common.utils;
...
public class TOTPGeneratorExample {
	...
	private static final String SECRET_KEY_FILE = "******TotpSecretKey.txt"; // Substituir pelo ficheiro que
																				// contém a secretKey

	public static void main(String[] args) {

		byte[] secretKey = Base64.getDecoder().decode(loadTotpSecretKey());
		String totp = new String(generateTOTP(secretKey));
		System.out.println(totp);
	}

	public static byte[] generateTOTP(byte[] secretKey) {
	...
	private static String loadTotpSecretKey() {
		try {
			String text = Files.readString(Paths.get(SECRET_KEY_FILE));
			return text;
		} catch (Exception e) {
